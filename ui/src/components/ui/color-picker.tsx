/**
 * Color Picker — adapted from Dice UI (https://diceui.com/docs/components/radix/color-picker).
 *
 * Source mirrors the Dice UI registry payload (`ui/color-picker.tsx`) so we
 * stay in sync with upstream semantics, but:
 *   - we resolve `@diceui/use-as-ref`, `@diceui/use-isomorphic-layout-effect`
 *     and `@diceui/use-lazy-ref` to the local implementations in
 *     `../../hooks/diceui/*` (those packages are not published on npm).
 *   - we resolve the upstream Popover/Slider/Select/Button/Input/VisuallyHidden
 *     to the local primitives under `./popover`, `./slider`, `./select`,
 *     `./button`, `./input`, and `../visually-hidden-input` so styling
 *     matches the rest of the design system.
 *
 * The component itself is a drop-in replacement for the diceui export: it
 * still exposes `ColorPicker`, `ColorPickerTrigger`, `ColorPickerContent`,
 * `ColorPickerArea`, `ColorPickerSwatch`, `ColorPickerHueSlider`,
 * `ColorPickerSaturationSlider`, `ColorPickerAlphaSlider`,
 * `ColorPickerEyeDropper`, `ColorPickerFormatSelect`, `ColorPickerInput`,
 * plus the `useColorPicker` store hook and `ColorPickerProps` type.
 */
...
import { useAsRef } from '../../hooks/diceui/use-as-ref';
import { useIsomorphicLayoutEffect } from '../../hooks/diceui/use-isomorphic-layout-effect';
import { useLazyRef } from '../../hooks/diceui/use-lazy-ref';
...
const SATURATION_SLIDER_NAME = 'ColorPickerSaturationSlider';
...
function ColorPickerHueSlider(
  props: React.ComponentProps<typeof SliderPrimitive>,
) {
  ...
  return (
    ...
    >
      ...
      <SliderPrimitive.Thumb
        ...
        className="block size-4 rounded-full border-2 border-[var(--card)] bg-[var(--foreground)] shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--page)] disabled:pointer-events-none disabled:opacity-50"
      />
    </SliderPrimitive.Root>
  );
}

function ColorPickerSaturationSlider(
  props: React.ComponentProps<typeof SliderPrimitive>,
) {
  const { className, ...sliderProps } = props;

  const context = useColorPickerContext(SATURATION_SLIDER_NAME);
  const store = useStoreContext(SATURATION_SLIDER_NAME);
  ...
  // Build a horizontal gradient from fully desaturated (gray at the current
  // value) → fully saturated at the current hue + value so the user can
  // preview what they're picking. Mirrors the alpha-slider visual treatment:
  // checkerboard base + colored gradient overlay.
  const hue = hsv?.h ?? 0;
  const value = hsv?.v ?? 0;
  const saturated = hsvToRgb({ h: hue, s: 100, v: value, a: 1 });
  const desaturated = hsvToRgb({ h: hue, s: 0, v: value, a: 1 });
  const trackBackground = `linear-gradient(to right, rgb(${desaturated.r}, ${desaturated.g}, ${desaturated.b}), rgb(${saturated.r}, ${saturated.g}, ${saturated.b}))`;

  return (
    <SliderPrimitive.Root
      data-slot="color-picker-saturation-slider"
      {...sliderProps}
      max={100}
      step={1}
      className={cn(
...
// ---------------- Swatch / EyeDropper / FormatSelect / Input ----------------
...
function ColorPickerEyeDropper(props: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      disabled={isDisabled}
    >
      {children ?? <PipetteIcon className="size-4" />}
    </Button>
  );
}
...
const inputGroupItemVariants = cva(
  'h-8 [-moz-appearance:textfield] focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-[var(--ring)] [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none',
  {
    variants: {
      position: {
